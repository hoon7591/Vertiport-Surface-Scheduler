...
def visualize_gantt(solution, vehicle_ids, *arg):
    ...
    if solver_type in ["exact", "exact_RHC", "FCFS_SAT", "FCFS_Gurobi", "FCFS_landing_SAT", "FCFS_landing_Gurobi"]:
        ax.set_title(f"Resource-Centric Gantt | Obj: {obj_val - tardiness_correction:.2f}, "
                     ...
                     f"Num Vehicles: {num_vehicles}, Weights: [{solution.objective_weights[0]:.2f}, {solution.objective_weights[1]:.2f}], "
                     f"Solver: {solver_type}, Obj Option: {solution.objective_option}\n"
                     f"Total T: {solution.total_tardiness:.2f}min, Avg T: {solution.avg_total_tardiness:.2f}min, "
                     f"Avg AT: {solution.avg_arrival_tardiness:.2f}min, Avg DT: {solution.avg_departure_tardiness:.2f}min, "
                     f"Max AT: {solution.max_arrival_tardiness:.2f}min, Max DT: {solution.max_departure_tardiness:.2f}min, "
                     f"Max V-wise T: {solution.max_vehicle_wise_weighted_tardiness:.2f}min, Std V-wise T: {solution.std_vehicle_wise_weighted_tardiness:.2f}min "
                     f"Non Delay Rate: {solution.non_delayed_vehicles_rate:.2f}\n"
                     f"Obj N: {solution.total_weighted_sum_tardiness_nominal:.2f}min, Max At N: {solution.max_arrival_tardiness_nominal:.2f}min, "
                     f"Max Dt N: {solution.max_departure_tardiness_nominal:.2f}min, Max V-wise T N: {solution.max_vehicle_wise_weighted_tardiness_nominal:.2f}min, "
                     f"Std V-wise T N: {solution.std_vehicle_wise_weighted_tardiness_nominal:.2f}min, CV V-wise T N: {solution.cv_vehicle_wise_weighted_tardiness_nominal:.2f}, "
                     f"Gini V-wise T N: {solution.gini_vehicle_wise_weighted_tardiness_nominal:.2f}\n"
                     f"CV V-wise T: {solution.cv_vehicle_wise_weighted_tardiness:.2f}, Gini V-wise T: {solution.gini_vehicle_wise_weighted_tardiness:.2f}, "
                     f"Pad Util: {solution.pad_resource_occupation_rate:.2f}, Pad Eff: {solution.pad_effective_occupation_rate:.2f}, "
                     f"Pad Ineff: {solution.pad_ineffective_occupation_rate:.2f}, Gate Util: {solution.gate_resource_occupation_rate:.2f}, "
                     f"Gate Eff: {solution.gate_effective_occupation_rate:.2f}, Gate Ineff: {solution.gate_ineffective_occupation_rate:.2f}, "
                     f"Total Util: {solution.total_resource_occupation_rate:.2f}, Total Eff: {solution.total_effective_occupation_rate:.2f}, "
                     f"Total Ineff: {solution.total_ineffective_occupation_rate:.2f}\n"
                     f"Pad Loss: {solution.pad_unconstrained_waiting_time:.2f}min, Gate Loss: {solution.gate_unconstrained_waiting_time:.2f}min, "
                     f"Buffer Loss: {solution.buffer_unconstrained_waiting_time:.2f}min, Total Loss: {solution.sum_total_unconstrained_waiting_time:.2f}min, "
                     f"Cost Index: {solution.cost_of_delay_index:.2f}"
                     )
    else:
        ax.set_title(f"Resource-Centric Gantt | Obj: {obj_val - tardiness_correction:.2f}, "
                     f"Total AT: {solution.total_arrival_tardiness:.2f}min, Total DT: {solution.total_departure_tardiness:.2f}min, "
...
def visualize_gantt_plotly(
    ...
    if solver_type in ["exact", "exact_RHC", "FCFS_SAT", "FCFS_Gurobi", "FCFS_landing_SAT", "FCFS_landing_Gurobi"]:
        title_str = (
            ...
            f"Runtime: {solver_runtime:.2f}s, Sim End Time: {sim_end_time:.2f}min, "
            f"Num Vehicles: {num_vehicles}, Weights: [{solution.objective_weights[0]:.2f}, {solution.objective_weights[1]:.2f}], "
            f"Solver: {solver_type}, Obj Option: {solution.objective_option}<br>"
            f"Total T: {solution.total_tardiness:.2f}min, Avg T: {solution.avg_total_tardiness:.2f}min, "
            f"Avg AT: {solution.avg_arrival_tardiness:.2f}min, Avg DT: {solution.avg_departure_tardiness:.2f}min, "
            f"Max AT: {solution.max_arrival_tardiness:.2f}min, Max DT: {solution.max_departure_tardiness:.2f}min, "
            f"Max V-wise T: {solution.max_vehicle_wise_weighted_tardiness:.2f}min, Std V-wise T: {solution.std_vehicle_wise_weighted_tardiness:.2f}min "
            f"Non Delay Rate: {solution.non_delayed_vehicles_rate:.2f}<br>"
            f"Obj N: {solution.total_weighted_sum_tardiness_nominal:.2f}min, Max At N: {solution.max_arrival_tardiness_nominal:.2f}min, "
            f"Max Dt N: {solution.max_departure_tardiness_nominal:.2f}min, Max V-wise T N: {solution.max_vehicle_wise_weighted_tardiness_nominal:.2f}min, "
            f"Std V-wise T N: {solution.std_vehicle_wise_weighted_tardiness_nominal:.2f}min, CV V-wise T N: {solution.cv_vehicle_wise_weighted_tardiness_nominal:.2f}, "
            f"Gini V-wise T N: {solution.gini_vehicle_wise_weighted_tardiness_nominal:.2f}<br>"
            f"CV V-wise T: {solution.cv_vehicle_wise_weighted_tardiness:.2f}, Gini V-wise T: {solution.gini_vehicle_wise_weighted_tardiness:.2f}, "
            f"Pad Util: {solution.pad_resource_occupation_rate:.2f}, Pad Eff: {solution.pad_effective_occupation_rate:.2f}, "
            f"Pad Ineff: {solution.pad_ineffective_occupation_rate:.2f}, Gate Util: {solution.gate_resource_occupation_rate:.2f}, "
            f"Gate Eff: {solution.gate_effective_occupation_rate:.2f}, Gate Ineff: {solution.gate_ineffective_occupation_rate:.2f}, "
            f"Total Util: {solution.total_resource_occupation_rate:.2f}, Total Eff: {solution.total_effective_occupation_rate:.2f}, "
            f"Total Ineff: {solution.total_ineffective_occupation_rate:.2f}<br>"
            f"Pad Loss: {solution.pad_unconstrained_waiting_time:.2f}min, Gate Loss: {solution.gate_unconstrained_waiting_time:.2f}min, "
            f"Buffer Loss: {solution.buffer_unconstrained_waiting_time:.2f}min, Total Loss: {solution.sum_total_unconstrained_waiting_time:.2f}min, "
            f"Cost Index: {solution.cost_of_delay_index:.2f}"
        )
